...
```
```
st_buffer does not correctly buffer longitude/latitude datatibble
...
```
q_obs has been converted to mm.
output/meteo-data-posto-74.RDS
...
```{r ncname-fake, eval = FALSE, echo = TRUE}
dest_dir <- "output"
forcings_ncname <- file.path(
  dest_dir, 
  paste(suffix, "input.nc", sep = "_")
