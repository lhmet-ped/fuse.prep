...
#univar_flowchart = "
graph TD
    A1[(CON Hydrosheds)] -->|sa_con_3s_hydrosheds.gr*| A2[extract_condem]
    ...
    C3 -->|poly_station| A2
    C3 -->|poly_station| F[annual_climatology]
    C3 -->|poly_station| C4[centroids]
    C4 --> |ccords| Z
    
    B1[(Dados climaticos Brasil)] --> |brick-prec-25km-19800101-20151231.nc| B2[import_nc]
    B2 --> |meteo_brick| F
    F --> |meteo_raster| Z
...
knitr::include_graphics("mermaid-diagram-20201018162221.svg")
```
...
O pré-processamento de dados requeridos pela função que cria o arquivo NetCDF de bandas de elevação (`elev_bands_nc()`) consiste nos processos representados pelos retângulos com linha mais grossa na Figura \@ref(fig:fluxograma). Este exemplo de pré-processamento são do posto da **UHE Governador Bento Munhoz da Rocha Neto**, identificada pelo código 74, e incluem:
...
Agora nós vamos mostrar como obter os dados listados na seção anterior.
